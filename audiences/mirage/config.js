...
export default function () {
  ...
  this.get('/audienceFolders', function(db, request) {
    var parent_id = Ember.isEmpty(request.queryParams.parent_id) ? null : parseInt(request.queryParams.parent_id),
        folder_id = Ember.isEmpty(request.queryParams.folder_id) ? null : request.queryParams.folder_id,
        responseData;
    
    if (folder_id === null) {
      let audienceFilteredFolders = audienceFolders.filter(function(i) {
        return i.parent === parent_id;
      });
      responseData = audienceFilteredFolders;
    } else {
      let audienceFilteredFolders = audienceFolders.filter(function(i) {
        return i.id === parseInt(folder_id);
      });
      responseData = audienceFilteredFolders;
    }
  ...
  this.get('/audiences', function (db, request) {
    var folder_id = Ember.isEmpty(request.queryParams.folder_id) ? null : request.queryParams.folder_id,
        filteredAudiences;

    if (folder_id) {
      filteredAudiences = audiences.filter(function(i) {
          return i.folder === parseInt(folder_id);
      });
    } else {
